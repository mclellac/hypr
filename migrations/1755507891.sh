...
echo "Migrating to use hypr-launch-webapp and hypr-launch-browser"
for desktop_file in ~/.local/share/applications/*.desktop; do
  if grep -q 'Exec=chromium --new-window --ozone-platform=wayland --app=' "$desktop_file"; then
    ...
    if [[ -n "$url" ]]; then
      sed -i "s|^Exec=.*|Exec=hypr-launch-webapp \"$url\"|" "$desktop_file"
    fi
  fi
done
...
if [ -f "$HYPR_BINDINGS_FILE" ]; then
  sed -i 's/\$browser =.*chromium.*$/\$browser = hypr-launch-browser/' "$HYPR_BINDINGS_FILE"
  sed -i 's/\$webapp="/hypr-launch-webapp "/g' "$HYPR_BINDINGS_FILE"
