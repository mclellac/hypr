...
# Users can still opt-in to Cloudflare DNS using: hypr-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-hypr-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-hypr-dns.network
  ...
  echo "To use Cloudflare DNS, run: hypr-setup-dns Cloudflare"
fi