...
def audit_theme(theme_name):
        ...
        with open(chrom_path, 'r') as f:
            ...
            if re.match(r'^\d+,\d+,\d+$', content):
                ...
                if canon not in hexes and canon not in found_colors:
                    ...
                    rows.append({'name': name, 'hex': hex_val, 'desc': "Detected in chromium.theme"})
    ...
    for root, dirs, files in os.walk(theme_path):
        for file in files:
            ...
            if file.endswith(('.png', '.jpg', '.jpeg', '.lock', '.pyc')): continue
            ...
            for match in HEX_COLOR_RE.finditer(content):
                ...
                if canon not in hexes and canon not in found_colors:
                         ...
                         for line in f_lines:
                             if raw_hex in line:
                                 if file.endswith('.css'):
                                     ...
                                     if m:
                                         n = m.group(1)
                                         if n.startswith('color'): name = f"term_{n}"
                                         else: name = n
                    ...
                    rows.append({'name': name, 'hex': raw_hex.lower(), 'desc': f"Detected in {file}"})
                    found_colors.add(canon)
                    hexes[canon] = name
            ...
            for match in HYPR_COLOR_RE.finditer(content):
                ...
                if canon not in hexes and canon not in found_colors:
                    ...
                    rows.append({'name': name, 'hex': hex_val.lower(), 'desc': f"Detected in {file}"})
                    found_colors.add(canon)
                    hexes[canon] = name
            ...
            for match in RGB_RE.finditer(content):
                ...
                if canon not in hexes and canon not in found_colors:
                    name = f"unknown_{canon.replace('#', '')}"
                    rows.append({'name': name, 'hex': hex_val.lower(), 'desc': f"Detected in {file} (from rgb/rgba)"})
                    found_colors.add(canon)
                    hexes[canon] = name
